      ...
      <SidebarInset className="relative flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden bg-[#0f1115]">
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            <div
              className={cn('min-h-0 flex-1', runtimeTemplate ? 'flex flex-row' : 'flex flex-col')}
            >
              <div
                className={cn(
                  ...
                  <div className="flex h-10 min-h-10 items-center justify-between border-b border-[#1c1f26] bg-[#0b0d11] px-3">
                    <div className="flex items-center gap-2 text-[11px] text-[#5c6370]">
                      ...
                      <span className="text-[#7d8596]">
                        {activeOpenFile ? getFileName(activeOpenFile.id) : 'No file open'}
                      </span>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          setEditorPreferences((previous) => ({ ...previous, theme: 'dark' }))
                        }
                        className={cn(
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setEditorPreferences((previous) => ({ ...previous, theme: 'light' }))
                        }
                        className={cn(
                          'h-8 rounded-md border px-3 text-xs transition-colors',
          ...
          <PlaygroundAiSidebar isOpen={isAiChatOpen} onClose={() => setIsAiChatOpen(false)} />
        </div>
      </SidebarInset>
    </div>
  )
