...
function inferTemplateFromPaths(
    paths: string[],
): 'REACT' | 'NEXTJS' | 'EXPRESS' | 'VUE' | 'ANGULAR' | 'HONO' {
...
async function createPlaygroundFromGithubRepo(repoUrl: string) {
    ...
    if (!treeResponse.ok) {
        throw new Error(
            'Failed to fetch repository tree. Make sure the repo is public and URL is correct',
        )
    }
    ...
    for (const entry of fileEntries) {
        const fileResponse = await fetch(
            `${apiBase}/contents/${encodeURIComponent(entry.path).replace(/%2F/g, '/')}?ref=${encodeURIComponent(branch)}`,
            {
                headers,
                cache: 'no-store',
            },
