...
const useFileExplorerStore = create<FileExplorerState>((set, get) => ({
    ...
    deleteFile: (filePath) => {
        set((state) => {
            ...
            const fallbackActive = activeFileWasDeleted
    ...
    deleteFolder: (folderPath) => {
        set((state) => {
            ...
            const activeWasDeleted = Boolean(
                state.activeFileId &&
                (state.activeFileId === folderPath ||
                    state.activeFileId.startsWith(`${folderPath}/`)),
            )
            const fallbackActive = activeWasDeleted
                ? (nextOpenFiles[0]?.id ?? null)
                : state.activeFileId
            const fallbackEditorContent = fallbackActive
                ? (nextOpenFiles.find((file) => file.id === fallbackActive)?.content ?? '')
    ...
    renameFile: (filePath, filename, extension) => {
        ...
        const cleanExtension = sanitizeNodeName(extension).replace(/^\./, '')
        if (!cleanFilename) return
        const updatedFileName = cleanExtension
            ? `${cleanFilename}.${cleanExtension}`
            : cleanFilename
        ...
        set((state) => {
            ...
            const nextActiveFileId =
                state.activeFileId === filePath ? renamedNode.path : state.activeFileId
