...
function isFileTreeNode(value: unknown): value is FileTreeNode {
    ...
    return typeof node.name === 'string' && typeof node.path === 'string' && hasValidType
}
...
async function saveTemplateToJson(
    ...
    const { encoding = 'utf-8', prettyPrint = true, createDirectoryIfMissing = true } = options
    ...
    const json = prettyPrint ? JSON.stringify(template, null, 2) : JSON.stringify(template)
